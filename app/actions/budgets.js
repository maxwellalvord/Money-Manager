...
import { deleteRecurringByBudgets } from './recurring'
...
export async function getBudgetsWithSpend() {
  ...
  return db.select({
    ...
    totalItem: sql`count(case when ${Expenses.isOverride} != 2 then ${Expenses.id} end)`.mapWith(Number),
  }).from(Budgets)
...
export async function updateSavingsGoal(id, goal) {
  ...
  await db.update(Budgets)
    .set({ savingsGoal: goal ? String(goal) : null })
...
export async function deleteAllNonSavingsBudgets() {
  ...
  await deleteRecurringByBudgets(ids)
...
export async function deleteBudgetWithExpenses(id) {
  ...
  await deleteRecurringByBudgets([Number(id)])
